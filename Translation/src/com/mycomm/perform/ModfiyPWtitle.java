            ...
            if ( file.isDirectory() ){                
        		...
        		try {        			       			
        			...
        			for(int i=0; i<list.size();i++){        				    
        				...
        				for(String st:set){ 
        					if(language.equals("en")){
        						if(prop1.getProperty(st).equals(str)&&prop1.getProperty(st)!=null){	//1.是中文   2.是英文        						        						
            						Element book=el.getParent().addElement("PWTitle");	               
        					...
        					if(language.equals("zh")){
        						if(prop2.getProperty(st).equals(str)&&prop2.getProperty(st)!=null){       						      						
            						Element book=el.getParent().addElement("PWTitle");	               
